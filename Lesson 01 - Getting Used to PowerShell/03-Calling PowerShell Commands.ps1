...
# You don't have to use the full parameter name. You only need to specify
# enough of the parameter name such that it's unique.
# Also, parameter names are not case sensitive.
Start-Process -fil powershell -arg '-command exit'

# Instead of calling a command using named parameters, you can rely on positional parameters.
# However, this requires foreknowledge of the parameter positions. You're much safer using 
# named parameters, and your code will be easier for yourself and other members of your team
# to read and make changes to.

# Use positional parameters to call a command. The "first" parameter on Start-Process is
# the -FilePath parameter, pointing to the binary we want to invoke.
...
# Some commands have "switch" parameters, which means they're either enabled or disabled. You don't actually pass
# a value into switch parameters.

# Example: The Start-Process command has a -Wait switch parameter, that pauses execution until the launched process
#          has completed.
...
# If you have an ambiguous command name, you can disambiguate by specifying the
# containing module as a prefix.
...
# PowerShell Splatting is really easy to use, and is supported starting with PowerShell v3.0. Simply build a HashTable
# (Dictionary) of key-value pairs, and then use the @ character to reference the variable name that contains the
# HashTable during a command invocation.
# If you encounter a "switch" parameter, set it to [boolean] $true or $false to enable / disable it.
...
# PowerShell commands support the notion of multiple "Parameter Sets". We can use the reflection capabilities
# of PowerShell to find out which commands have multiple Parameter Sets.
# NOTE: We'll talk more about dynamic objects later on.
...
# It looks like Split-Path has the most Parameter Sets at 8.
# Let's explore the different command syntaxes we can use.
